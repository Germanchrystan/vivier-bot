...
func engraveProgression(progression []composer.Chord) (string, string) {
	...
	for index, chord := range progression {
		...
		if index == len(progression)-1 {
			...
			remainingBeats := strings.Repeat("<note><rest/><duration>1</duration><voice>1</voice><type>quarter</type></note>", remainingBeatQuantity)

			basePart += remainingBeats
			upperPart += remainingBeats
		}
	}
